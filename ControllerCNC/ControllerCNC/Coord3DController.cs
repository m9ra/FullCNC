...
{
    public class Coord3DController
    {
        ...
        private volatile int _desiredDirectionV;
        ...
        private volatile int _deltaTV;

        private volatile bool _stop = true;


        public Coord3DController(DriverCNC cnc)
        {
            _cnc = cnc;
        ...
        private void sendNextPlan()
        {
            ...
            var accelerationZ = CreateAcceleration(_deltaTV, _desiredSpeed * _desiredDirectionV);
            ...
            if (accelerationX.Any() && accelerationY.Any() && accelerationZ.Any())
            {
                for (var i = 0; i < accelerationX.Length; ++i)
                {
                    sendInstruction(accelerationX[i], accelerationY[i], accelerationZ[i]);
                }
            ...
            if (accelerationX.Any() || accelerationY.Any() || accelerationZ.Any())
            {
                ...
                {
                    sendInstruction(acceleration, null, null);
                }

                foreach (var acceleration in accelerationY)
                {
                    sendInstruction(null, acceleration, null);
                }

                foreach (var acceleration in accelerationZ)
                {
                    sendInstruction(null, null, acceleration);
                }
            ...
            velocityReached();
            if (_deltaTX == 0 && _deltaTY == 0 && _deltaTV == 0)
            {
                _stop = true;
                return;
            }
            ...
            var timedStepCountZ = _deltaTV == 0 ? 0 : instructionTime / Math.Abs(_deltaTV);

            var stepCountX = (Int16)(timedStepCountX * Math.Sign(_deltaTX));
            var stepCountY = (Int16)(timedStepCountY * Math.Sign(_deltaTY));
            var stepCountZ = (Int16)(timedStepCountZ * Math.Sign(_deltaTV));
            ...
            var zInstruction = new ConstantInstruction(stepCountZ, (UInt16)Math.Abs(_deltaTV), 0);

            sendInstruction(xInstruction, yInstruction, zInstruction);
        }

        private void sendInstruction(StepInstrution planeInstruction1, StepInstrution planeInstruction2, StepInstrution planeInstruction3)
        {
        ...
        private void velocityReached()
        {
            ...
            _deltaTV = _desiredDirectionV * _desiredSpeed;
        }

        public void SetMovement(int dirX, int dirY, int dirV)
        {
            _desiredDirectionX = dirX;
            _desiredDirectionY = dirY;
            _desiredDirectionV = dirV;
            _stop = false;
        }
